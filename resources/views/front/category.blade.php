...
<script>
  ...
  <div class="spinner-grow text-light"></div>`)
        $.ajax({
            url: `{{URL::to('filter-product')}}?cat=${cat}&mode=${mode}&subcat=0&page=${val}&val=${$("#val").val()}&key=${$("#sort-by").val()}`,
            success: function(result) {
                $("#categoryContainer").html(result);
            }
        });
  ...
  <div class="spinner-grow text-light"></div>`)
        $.ajax({
            url: `{{URL::to('filter-product')}}?mode=${mode}&cat={{$category->fid}}&subcat=0&val=""&key=${$("#sort-by").val()}`,
            success: function(result) {
                $("#categoryContainer").html(result);
            }
        });
